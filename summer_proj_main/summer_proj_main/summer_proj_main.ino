...
int IRStatus = 0; // 0: none, 1: 312, 2: 547
...
<html>
    ...
    <div class="layout">
      ...
      <div class="column">
        ...
        <div class="pad">
          ...
          <div class="pad-hint">USE W A S D TO DRIVE &middot; R TOGGLES RADIO</div>
        </div>
        ...
        <!-- ─── Per-motor trim sliders (0.0–1.0, step 0.1) ─── -->
        <div class="turnratio">
          <div class="turnratio-head">
            <span class="label">Left Motor</span>
            <span id="lmod_val">1.0</span>
          </div>
          <input type="range" id="lmod_slider" min="0" max="1" step="0.1" value="1">
        </div>

        <div class="turnratio">
          <div class="turnratio-head">
            <span class="label">Right Motor</span>
            <span id="rmod_val">0.9</span>
          </div>
          <input type="range" id="rmod_slider" min="0" max="1" step="0.1" value="0.9">
        </div>
      </div>
  ...
  document.addEventListener("keydown", (e) => {
    ...
    // R toggles the radio receiver (same action as clicking the button)
    if (k === "r") {
      toggleRadio();
      return;
    }
  ...
  // ─── Per-motor trim sliders ───
  const lmodSlider = document.getElementById("lmod_slider");
  const lmodOut = document.getElementById("lmod_val");
  let lmodTimer = null;
  lmodSlider.addEventListener("input", (e) => {
    lmodOut.innerText = e.target.value;
    clearTimeout(lmodTimer);
    lmodTimer = setTimeout(() => {
      send("/leftmod?value=" + e.target.value, null);
    }, 80);
  });

  const rmodSlider = document.getElementById("rmod_slider");
  const rmodOut = document.getElementById("rmod_val");
  let rmodTimer = null;
  rmodSlider.addEventListener("input", (e) => {
    rmodOut.innerText = e.target.value;
    clearTimeout(rmodTimer);
    rmodTimer = setTimeout(() => {
      send("/rightmod?value=" + e.target.value, null);
    }, 80);
  });
...
// ─── Per-motor trim handlers ───
// GET /leftmod?value=0.0..1.0  → scales the LEFT motor PWM
void handleLeftMod() {
  if (server.hasArg("value")) {
    ...
    if (v > 1.0) v = 1.0;
    leftMod = v;
    Serial.print(F("leftMod = "));
    Serial.println(leftMod);
  }
  server.send(200, F("text/plain"), String(leftMod, 2));
}

// GET /rightmod?value=0.0..1.0  → scales the RIGHT motor PWM
void handleRightMod() {
  if (server.hasArg("value")) {
    float v = server.arg("value").toFloat();
    if (v < 0.0) v = 0.0;
    if (v > 1.0) v = 1.0;
    rightMod = v;
    Serial.print(F("rightMod = "));
    Serial.println(rightMod);
  }
  server.send(200, F("text/plain"), String(rightMod, 2));
}
...
void back() {
  ...
  analogWrite(LEFT_PWM, speed * leftMod);
  digitalWrite(RIGHT_DIR, LOW);
  analogWrite(RIGHT_PWM, speed * rightMod);
  Serial.println("BACK");
